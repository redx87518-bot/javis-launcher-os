...
data class SettingsUiState(
    ...
    val elevenLabsKey: String = "", val voiceId: String = "EXAVITQu4vr4xnSDxMaL",
    val voiceSpeed: Float = 1.0f, val voiceStability: Float = 0.5f, val useTts: Boolean = false,
    ...
    val groqTestResult: String = "",
    val openRouterTestResult: String = "",
    val elevenLabsTestResult: String = "",
    val isTestingGroq: Boolean = false,
    val isTestingOpenRouter: Boolean = false,
...
class SettingsViewModel @Inject constructor(
    ...
    fun onVoiceStabilityChange(v: Float) = _uiState.update { it.copy(voiceStability = v) }
    ...
    fun testGroq() {
        val s = _uiState.value
        if (s.groqKey.isBlank()) { _uiState.update { it.copy(groqTestResult = "❌ No Groq API key entered") }; return }
        viewModelScope.launch {
            _uiState.update { it.copy(isTestingGroq = true, groqTestResult = "Testing Groq…") }
            // Temporarily save key for test
            preferences.saveAiProviderPrefs(preferences.getAiProviderPrefs().copy(groqKey = s.groqKey, groqModel = s.groqModel, groqEnabled = true))
            val result = brainManager.testProvider(AiProvider.GROQ)
            _uiState.update { it.copy(isTestingGroq = false, groqTestResult = result.getOrElse { e -> "❌ ${e.message?.take(100)}" }) }
        }
    }

    fun testOpenRouter() {
        val s = _uiState.value
        if (s.openRouterKey.isBlank()) { _uiState.update { it.copy(openRouterTestResult = "❌ No OpenRouter API key entered") }; return }
        viewModelScope.launch {
            _uiState.update { it.copy(isTestingOpenRouter = true, openRouterTestResult = "Testing OpenRouter…") }
            preferences.saveAiProviderPrefs(preferences.getAiProviderPrefs().copy(openRouterKey = s.openRouterKey, openRouterEnabled = true))
            val result = brainManager.testProvider(AiProvider.OPENROUTER)
            _uiState.update { it.copy(isTestingOpenRouter = false, openRouterTestResult = result.getOrElse { e -> "❌ ${e.message?.take(100)}" }) }
        }
    }

    fun testElevenLabs() {
        val s = _uiState.value
        if (s.elevenLabsKey.isBlank()) { _uiState.update { it.copy(elevenLabsTestResult = "❌ No ElevenLabs key entered") }; return }
        viewModelScope.launch {
            _uiState.update { it.copy(isTestingElevenLabs = true, elevenLabsTestResult = "Testing ElevenLabs…") }
            val result = voiceManager.elevenLabsClient.testConnection(s.elevenLabsKey, s.voiceId.ifBlank { "EXAVITQu4vr4xnSDxMaL" })
            _uiState.update { it.copy(isTestingElevenLabs = false, elevenLabsTestResult = result.getOrElse { e -> "❌ ${e.message?.take(100)}" }) }
        }
    }

    fun testVoice() {
        val s = _uiState.value
        if (s.elevenLabsKey.isBlank()) { _uiState.update { it.copy(elevenLabsTestResult = "❌ No ElevenLabs key — using TTS") }; return }
        viewModelScope.launch {
            _uiState.update { it.copy(isTestingVoice = true, elevenLabsTestResult = "Playing test voice…") }
            val result = voiceManager.elevenLabsClient.testSpeak(s.elevenLabsKey, s.voiceId.ifBlank { "EXAVITQu4vr4xnSDxMaL" })
            _uiState.update { it.copy(isTestingVoice = false, elevenLabsTestResult = result.getOrElse { e -> "❌ ${e.message?.take(100)}" }) }
        }
    }
