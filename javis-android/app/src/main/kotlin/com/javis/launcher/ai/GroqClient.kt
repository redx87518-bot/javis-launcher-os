...
class GroqClient @Inject constructor(
    ...
    suspend fun chat(messages: List<AiMessage>, system: String): String = withContext(Dispatchers.IO) {
        ...
        if (prefs.groqKey.isBlank()) throw IllegalStateException("Groq API key not configured")

        val msgs = mutableListOf<Map<String, String>>()
        msgs.add(mapOf("role" to "system", "content" to system))
        messages.takeLast(20).forEach { msgs.add(mapOf("role" to it.role, "content" to it.content)) }

        val response = api.chat(
            auth = "Bearer ${prefs.groqKey}",
            request = OpenRouterRequest(
                model = prefs.groqModel.ifBlank { "llama-3.1-8b-instant" },
                messages = msgs,
                maxTokens = 512,
                temperature = 0.7f
            )
